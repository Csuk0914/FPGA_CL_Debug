...
#ifndef GALOISGPU_APPS_PR_PAGERANKPUSH_H_
#define GALOISGPU_APPS_PR_PAGERANKPUSH_H_
...
void PR_Push(int num_steps, char * fname) {
   using namespace Galois::OpenCL;
   typedef Galois::OpenCL::LC_LinearArray_Graph<NodeDataPush, unsigned int> Graph;

   fprintf(stderr, "Launching PageRank\n");
   Graph g;
   cl_event event;

   g.read(fname);
   for (size_t i = 0; i < g.num_nodes(); ++i) {
      NodeDataPush * d = &g.node_data()[i];
      d->nout = 0;
      d->residual = 0;
      d->value = 0;
   }
   g.copy_to_device();
   cl_kernel init_kernel = env.kernel1; //("apps/pr/PageRankPush.cl", "initialize");
   cl_kernel pr_kernel = env.kernel2; //("apps/pr/PageRankPush.cl", "pageRank");
   ...
   fprintf(stderr, "Completed PageRankPush successfully!\n");
}

#endif /* GALOISGPU_APPS_PR_PAGERANKPUSH_H_ */
